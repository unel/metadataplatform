...
based-on: tasks/0002-store-crud/stages/02-acceptance/01-write/report-001.md
see-also: tasks/0002-store-crud/stages/02-acceptance/03-fix/report-001.md
review-source: tasks/0002-store-crud/stages/02-acceptance/02-review/report-002.md
status: done
agent: Танк
---
...
### Сценарий AC-FS-INIT-03: Логирование успешной инициализации FS-стора

**Given** конфиг содержит `basedir="/tmp/teststore"`, логгер подключён к конструктору
**When** вызван конструктор `fs.New(cfg)` и инициализация завершилась успешно
**Then** в лог записана строка уровня INFO содержащая текст `store/fs initialized` и поле `basedir=/tmp/teststore`

---

### Сценарий AC-FS-INIT-04: Логирование ошибки инициализации FS-стора

**Given** конфиг содержит `basedir` — путь к директории которую нельзя создать; логгер подключён к конструктору
**When** вызван конструктор `fs.New(cfg)` и `os.MkdirAll` вернул ошибку
**Then** в лог записана строка уровня ERROR содержащая текст `store/fs init failed`, поле `basedir=<path>` и поле `error=<текст ошибки>`

---

## Блок 2: EntityStore — Upsert

### Сценарий AC-ENTITY-UPSERT-01: Создание новой entity — входящие timestamps игнорируются

**Given** FS-стор инициализирован, запись с `id="ent-1"` не существует; входящий объект содержит `CreatedAt=time.Date(2000,1,1,0,0,0,0,time.UTC)` и `UpdatedAt=time.Date(2000,1,1,0,0,0,0,time.UTC)`
**When** вызван `EntityStore.Upsert(ctx, Entity{ID:"ent-1", Type:"service", CreatedAt: time.Date(2000,1,1,0,0,0,0,time.UTC), UpdatedAt: time.Date(2000,1,1,0,0,0,0,time.UTC)})`
**Then** возвращает `nil`; файл `entities/ent-1.json` существует на диске; `created_at` в сохранённом файле не равен `2000-01-01T00:00:00Z`; `updated_at` в сохранённом файле не равен `2000-01-01T00:00:00Z`; оба поля установлены в UTC-время вызова
...
### Сценарий AC-ROUTER-09: Невалидный JSON — поток испорчён, соединение закрывается, ошибка логируется

**Given** роутер инициализирован с логгером, соединение установлено
**When** роутер получает строку `{not-valid-json`
**Then** роутер закрывает соединение; дальнейшие запросы в этом соединении не обрабатываются; в лог записана строка уровня ERROR содержащая поле `op=unknown` и поле `parse_error` с текстом ошибки парсинга
...
**When** вызван `uuid.NewV7()` и результат передан как `id` в `EntityStore.Upsert`
**Then** функция `uuid.NewV7()` возвращает непустую строку в формате UUID; операция Upsert завершается успешно
...
- [x] AR-1: AC-ROUTER-09 дополнен проверкой лог-записи ERROR с полем parse_error
- [x] AR-2: добавлены AC-FS-INIT-03 и AC-FS-INIT-04 — логирование инициализации
- [x] AR-3: AC-NFT-ATOMIC-02 переписан как поведенческий — статическая проверка кодовой базы убрана
- [x] AR-4: AC-ENTITY-UPSERT-01 — Given содержит конкретные timestamps, Then проверяет что сохранённые поля не равны заданным значениям
- [x] AR-5: AC-ROUTER-09 — Then дополнен проверкой поля `op=unknown` наряду с `parse_error`

## Changelog

### AC-FIX-1 (AR-1)
AC-ROUTER-09: дополнен Then — добавлена проверка лог-записи уровня ERROR с полем `parse_error`. Заголовок сценария обновлён: добавлено "ошибка логируется". Given дополнен: "с логгером".

### AC-FIX-2 (AR-2)
Блок 1: добавлены два новых сценария.
- AC-FS-INIT-03: успешная инициализация → лог INFO с `store/fs initialized` и `basedir=<path>`.
- AC-FS-INIT-04: ошибка инициализации → лог ERROR с `store/fs init failed`, `basedir=<path>`, `error=<text>`.
Given обоих сценариев содержит явный путь и логгер.

### AC-FIX-3 (AR-3)
AC-NFT-ATOMIC-02: Then переписан как поведенческий. Убрана фраза "в кодовой базе не использован". Новый Then: `uuid.NewV7()` возвращает непустую строку в формате UUID; переданная как `id` в `Upsert` — операция успешна.

### AC-FIX-4 (AR-4)
AC-ENTITY-UPSERT-01: сценарий переименован ("входящие timestamps игнорируются"). Given: Entity содержит явные `CreatedAt=time.Date(2000,1,1,0,0,0,0,time.UTC)` и `UpdatedAt=time.Date(2000,1,1,0,0,0,0,time.UTC)`. When: Entity передаётся с этими значениями. Then: проверяет что поля в сохранённом файле не равны `2000-01-01T00:00:00Z`, установлены в UTC-время вызова.

### AC-FIX-5 (AR-5)
AC-ROUTER-09: Then дополнен — добавлена проверка поля `op=unknown` наряду с `parse_error`. Оба поля теперь обязательны в лог-записи уровня ERROR.
...
- AC-ROUTER-09 vs AC-ROUTER-10: parse error (json.Decoder) закрывает соединение и логируется ERROR; invalid data в envelope — application-level, соединение сохраняется
- AC-RELATION-UPSERT-02: FS намеренно не проверяет FK — explicit ограничение из §7
- AC-NFT-ATOMIC-02: статическая проверка "uuid.New() не используется в кодовой базе" убрана как непроверяемая тестом — это code review пункт, не acceptance
