...
run: 2
date: 2026-04-28T20:18:02Z
created: 2026-04-28T20:18:02Z
see-also: tasks/0002-store-crud/stages/01-spec/03-fix/report-001.md
status: done
agent: Танк
...
- **CR-9**: §4.1 — failure mode инициализации: `os.MkdirAll` ошибка → конструктор возвращает ошибку. §НФТ-О-2 — лог-строка `ERROR store/fs init failed`. §НФТ-Н-5 — новый пункт надёжности.
- **CR-10**: §Контекст — структура пакетов: `store/`, `store/router/`, `store/fs/`. §5.0 — новый раздел с конструктором `router.New(entities, relations, jobs)`. §НФТ-С-4 — новый пункт сопровождаемости.
- **CR-11**: §НФТ-О-1 — формат лог-строки для `list`: `DEBUG op=list type=<type> count=<N>`. Поле `id` для list явно опускается.
- **CR-12**: §5.2 — добавлена строка "для `upsert` поле `id` в envelope игнорируется — `id` берётся из `data`".
- **CR-13**: §4.3 — добавлен явный шаг 3: выставить `created_at`/`updated_at` по правилам §3.1 перед encode.
...
- **Структура пакетов:** роутер в `store/router/`, FS в `store/fs/`, интерфейсы+модели в `store/store.go`. Роутер зависит от интерфейсов, не от реализаций.
- **Конструктор роутера:** `router.New(entities store.EntityStore, relations store.RelationStore, jobs store.JobStore) *Router`. Инъекция через конструктор.
- **id при upsert:** envelope `id` игнорируется, берётся из `data.id`.
- **Лог list:** `count=<N>` вместо `id=<id>` — операция не адресована.
- **FS init failure:** конструктор возвращает ошибку при провале `os.MkdirAll`. Стор без инициализации не используется.
