...
6. Временны́е метки `created_at` и `updated_at` выставляются в публичных методах (`EntityStore.Upsert`, `RelationStore.Upsert`, `JobStore.Upsert`) до вызова внутренней функции `upsert(ctx, path, obj)`. Внутренняя функция `upsert` получает объект с уже выставленными метками и записывает его как есть.
...
| Ошибка чтения существующей записи при upsert | `READ_ERROR` | `"failed to read existing record"` |
...
2. `defer tmp.Close()` — объявить первым, гарантирует закрытие файлового дескриптора при любой ошибке или panic.
3. `defer func() { if err != nil { os.Remove(tmp.Name()) } }()` — объявить вторым. Cleanup при любой ошибке или panic. Использование if-err-branch после каждого шага **запрещено**. Требует named return `(err error)` в сигнатуре. Порядок LIFO: Remove выполнится раньше Close.
4. Объект уже содержит выставленные `created_at`/`updated_at` (устанавливаются в вызывающем публичном методе по правилам §3.1). Функция `upsert` записывает объект как есть.
5. Encode объект в JSON, записать в temp-файл.
6. `tmp.Sync()` — fsync перед rename.
7. `os.Rename(tmp.Name(), targetPath)`.

Запреты:
- `os.WriteFile` запрещён — не атомарен.
- Temp-файл в `os.TempDir()` запрещён — может быть на другом filesystem.
- Явный `tmp.Close()` в теле функции после `tmp.Sync()` запрещён — закрытие через `defer tmp.Close()`.
...
FS-реализация **не защищает конкуре��тный доступ** в этой итерации.
...
При получении любой другой ошибки decode от `json.Decoder` (ошибка парсинга JSON) — поток испорчён, `json.Decoder` находится в неопределённом состоянии и не гарантирует восстановление. Роутер **закрывает соединение** и выходит из цикла. Логировать как ошибку: `ERROR op=unknown parse_error="<text>"`.
...
| `READ_ERROR` | ошибка чтения (get, или чтение существующей записи при upsert) |
...
Немедленная ошибка при неизвестном `op`, `type`, пустом `id`, отсутствующем или null `data` для upsert. После application-level ошибки соединение **не разрывается** — следующий запрос обрабатывается нормально.

**Исключение:** parse error при decode (`json.Decoder` вернул синтаксическую ошибку JSON) — поток испорчён. В этом случае соединение **закрывается** (см. §5.1).
...
- **CR-1—CR-16**: см. report-001.md, report-002.md, report-003.md
- **CR-17**: §4.3 — добавлен `defer tmp.Close()` как первый defer после создания temp-файла; порядок деклараций defer зафиксирован (Close первым, Remove вторым — LIFO). §НФТ-Н-3 — уточнён порядок деферов.
- **CR-18**: §3.1 Failure modes — добавлен `READ_ERROR` / `"failed to read existing record"` для ошибки чтения существующей записи при upsert. §5.5 — уточнено описание `READ_ERROR`.
- **CR-19**: §4.3 — шаг 3 переформулирован: временные метки выставляются в публичных методах до вызова `upsert()`, функция `upsert` получает готовый объект. §3.1 — добавлен явный пункт 6 о месте выставления меток.
- **CR-20**: §5.1 — добавлено: parse error в `json.Decoder` → поток испорчён → соединение закрывается, логируется как ERROR. §НФТ-Б-1 — разделено: application-level ошибки не разрывают соединение; parse error закрывает соединение.

## Решения принятые в процессе

- **defer f.Close() первым:** LIFO — Remove выполнится раньше Close, что корректно на всех платформах.
- **READ_ERROR при upsert:** семантически точнее WRITE_ERROR для ошибки чтения.
- **Временные метки снаружи:** `upsert` — IO-функция, не бизнес-логика. Публичные методы выставляют метки.
- **Parse error закрывает соединение:** поток JSON испорчён, восстановление не гарантировано. Application-level ошибки — другой уровень.
